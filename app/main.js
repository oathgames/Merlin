...
// Single gate for every OS-level link open. Renderer-originated URLs used
// to flow into shell.openExternal through two different paths — the
// new-window handler (filtered) and the right-click "Open Link" item
// (unfiltered) — which meant a crafted markdown link like file:///C:/… or
// javascript:… could trigger arbitrary OS protocol handlers from the
// context menu. All renderer-input call sites now share this helper.
...
async function createWindow() {
  ...
  // Codex P1 #6 subdirectory + extension allowlist for the merlin://
  // protocol. Before this was pinned, any file under appRoot was fetchable
  // from the renderer: crafted content could read CLAUDE.md, memory.md,
  // version.json, package.json, legacy per-brand plaintext configs
  // (.claude/tools/.merlin-config-*), and so on. The traversal guard only
  // rejected ../ escapes out of appRoot; it did not stop merlin://foo.md
  // from serving appRoot/foo.md. Add new subdirs here explicitly — the
  // default-deny is the load-bearing property.
  ...
  protocol.handle('merlin', async (request) => {
    ...
    // Subdir allowlist: renderer can only reach results/, assets/, pwa/.
    if (!ALLOWED_MERLIN_ROOTS.some((r) => filePath.startsWith(r))) {
      return new Response('Forbidden', { status: 403 });
    }

    // Extension allowlist: refuse anything not in MIME_TYPES rather than
    // falling through to application/octet-stream. No .md, .json, .log,
    // .js, .html — keeping this to image/video/audio/pdf only closes the
...
// Codex P1 #5: salt-aware derivation. nil salt = legacy v1 (used only to
// decrypt pre-upgrade vaults during one-time migration). 32-byte salt
// from .vault-salt = v2 (all new writes). MUST stay byte-compatible with
// vault.go:vaultKey(salt).
...
function vaultSave(map) {
  // Always write v2 (salted). First save after upgrading from v1
  // auto-generates .vault-salt and re-encrypts with the salted key.
...
// Defense-in-depth for every entrypoint that composes a filesystem path
// from a brand name. The Electron preload pre-validates renderer IPC args
// against BRAND_RE (see app/preload.js:51 + main.js earlier), but MCP
// tool schemas accept raw strings — a call like
// { name: "meta_setup_account", args: { brand: "../../evil" } } would
// flow straight into path.join(appRoot, ".claude", "tools",
// `.merlin-config-${brand}.json`) and write outside the tools directory.
// Applying the same regex here makes every path builder safe regardless
// of whether the caller is the renderer, the MCP stdio channel, or a test.
