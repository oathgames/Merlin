...
// binary-paths-update-race.test.js both seed fixed-name installers there, so
// each file's sweep reclaimed the other's fixture and the "still reclaims a
// genuinely stale installer" assertion failed at random. Give this process its
// own tmpdir before requiring binary-paths.js (os.tmpdir() reads TEMP/TMP/
// TMPDIR on every call, so nothing is cached).
{
  const realTmp = require('node:os').tmpdir();
  const mine = require('node:fs').mkdtempSync(require('node:path').join(realTmp, 'bp-proc-'));
