...
2. **Do all of this silently — no step narration, just show results:**
   ...
   **Brand registration — call `brand_activate` FIRST, BEFORE the scrape (dropdown-first).** The MOMENT you have the URL — before fetching the site, before writing brand.md — derive `<slug>` from the domain (`gymshark.com` → `gymshark`, `shop.acme.co.uk` → `acme`) and run `brand_activate({brand: "<slug>", displayName: "<Title-cased brand or domain>", url: "<url>"})`. Passing `displayName`/`url` tells the host to CREATE the brand folder with a stub `brand.md` + `memory.md` and switch to it immediately: the dropdown swaps to the new brand, connections / spells / perf bar refresh, and every subsequent bubble in this conversation is associated with the new brand's thread. **This is the single most important reliability step in onboarding** — if the scrape or any later step fails, the brand is already saved and switchable, so the user resumes instead of redoing the whole setup. Keep `<slug>` fixed for the life of the brand: the brand.md you write from the scrape below ENRICHES this stub (overwrites it with full data); it never renames the folder. If `brand_activate` returns `{ ok: false }` or throws, surface a single-line error — *"Couldn't create that brand: <reason>. Check the URL and try again."* — and STOP. The MCP call returns `{ summary, brand, previousBrand, created, manifestStatus? }`.
